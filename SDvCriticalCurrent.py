...
def EvenArrayOfJunctions(sigma, numOfJunctions, width, arrayJ = []): # Generates an array of junctions given some standard deviation & mean width
    ...
    if (len(arrayJ) != 0):
        ...
        for k in range(2, arrOfJunctions.size - 2, 2):
            arrOfJunctions[k] = arrayJ[k] - (arrOfWidthsDiv2[k//2 - 1]) # Even "expansion" of the junctions
            arrOfJunctions[k + 1] = arrayJ[k] + (arrOfWidthsDiv2[k//2 - 1]) # Even "expansion" of the junctions
        ...
        arrOfJunctions[1] = width + arrOfWidthsDiv2[0] # Intializes the first junction, necessary otherwise the for-loop breaks
        for k in range(2, arrOfJunctions.size - 2, 2):
            arrOfJunctions[k] = k//2 * junctionCenter - (width + arrOfWidthsDiv2[k//2 - 1]) # Even "expansion" of the junctions
            arrOfJunctions[k + 1] = k//2 * junctionCenter + (width + arrOfWidthsDiv2[k//2 - 1]) # Even "expansion" of the junctions
        arrOfJunctions[-2] = 1 - arrOfWidthsDiv2[-1] # Hard sets the last junction, also necessary otherwise for-loop breaks
        arrOfJunctions[-1] = 1 # Hard sets the last junction, not accounted for in the for-loop
...
def OddArrayOfJunctions(sigma, numOfJunctions, width, arrayJ = []): # Generates an array of junctions given some standard deviation & mean width
    ...
    # Creates the list of the terms generated by the delta, without accounting for the mean
    # NOT THE TRUE STANDARD DEVIATION - Need to account for both positives and minuses, this is just positive
    for i in range(numOfJunctions//2 + 1):
        arrOfWidthsDiv2[i - 1] = np.sqrt(i * delta)
    ...
    if (len(arrayJ) != 0):
        ...
        # For each junction in the inputted array
        for k in range(2, arrOfJunctions.size - 2, 2):
            arrOfJunctions[k] = arrayJ[k] - (arrOfWidthsDiv2[k//2]) # Even "expansion" of the junctions
            arrOfJunctions[k + 1] = arrayJ[k] + (arrOfWidthsDiv2[k//2]) # Even "expansion" of the junctions
        arrOfJunctions[-2] = arrayJ[-2] - arrOfWidthsDiv2[-1] # Hard sets the last junction, also necessary otherwise for-loop breaks
        arrOfJunctions[-1] = 1
    else:
        ...
        for k in range(2, arrOfJunctions.size - 2, 2):
            arrOfJunctions[k] = k//2 * junctionCenter - (width + arrOfWidthsDiv2[k//2 - 1]) # Even "expansion" of the junctions
            arrOfJunctions[k + 1] = k//2 * junctionCenter + (width + arrOfWidthsDiv2[k//2 - 1]) # Even "expansion" of the junctions
        arrOfJunctions[-2] = 1 - (arrOfWidthsDiv2[-1] + correctional_last_term) # Hard sets the last junction, also necessary otherwise for-loop breaks
        arrOfJunctions[-1] = 1 # Hard sets the last junction, not accounted for in the for-loop
    return arrOfJunctions
